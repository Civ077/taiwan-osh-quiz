...
| 程式碼 | https://github.com/Civ077/taiwan-osh-quiz （public，main）**只放網站** |
| 題庫原始檔 | https://github.com/Civ077/taiwan-osh-quiz-bank （**private**）：`題庫/`、xlsx 等含答案的檔案 |

## 答案不外流（v0.10 起）

答案**不再隨題庫下發到瀏覽器**。GAS `fields=play` 只給題目與選項，整局結束時
以 `act=grade` 一次批改回傳正解與解析（一次最多 25 題）。因此：

- `docs/data/questions.json` 不含 `answer`／`explain`
- 含答案的檔案（`題庫/`、`*.xlsx`）已從這個公開庫**停止追蹤**，改放私有庫
  `Civ077/taiwan-osh-quiz-bank`；本機仍保留，`build_sheet.py` 照常運作
- 答案的權威來源是 Google Sheet，只有 GAS 讀得到

⚠️ 尚未處理：這個公開庫的**歷史 commit 裡仍有舊版含答案的檔案**，要徹底清除
需重寫歷史並強制推送。

> 這是練習用的競賽遊戲，成績由瀏覽器計算後上傳，**不可作為教育訓練時數或
> 測驗合格紀錄**。
...
- 職安：使用者指定的 35 部法規（build_sheet.OSH_KEEP），其餘職安法規的題目檔保留於題庫但不納入建表。
...
| 8 | 環保子法（空污專責人員、排放標準、室內空品、噪音細則、水污細則、廢清細則、環評細則、毒化物、土污、氣候、環教…） | 80 | draft |
| 9 | 環保子法逐條（許可證、空污費、VOC、鍋爐、噪音、水體、飲用水、清除處理機構、回收再利用、環評認定／準則、毒化物 7 部、環境用藥、土水監測、減量交易、碳足跡、環教人員、環檢機構） | 1,118 | draft |
| 10 | 環保母法 25 部逐條（空污、噪音、水污、廢清、環評、海污、海岸、海保、野保、濕地、土污、毒化、環境基本、氣候、環教、資源循環、公害糾紛） | 1,603 | draft |
| 11 | 職安 35 部逐條補齊（設施規則、營造、高壓氣體、起重升降、鍋爐、危險性工作場所、化學品、衛生標準、勞基法、性平法、勞檢法…） | 1,454 | draft |
| 12 | 環保新增 110 部細則／標準／準則／管理辦法逐條（ENV-98～207） | 1,742 | draft |
| 13 | 環保子法（批次 8 之 28 部）逐條補齊 | 522 | draft |
| 7 | 環保第二、三層 | ~150 | 未開始 |
