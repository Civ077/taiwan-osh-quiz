 ...
 */
const GAS_VERSION = 8;   // v8：fields=play（不下發答案與解析）＋ act=grade（整局結束才批改）   // v5：append/range 可自動建立分頁（create:true）；新增 clear_from 模式（刪除第 N 列以後）
...
function doGet(e) {
  ...
  if (String(p.act || '') === 'grade') return grade_(p);       // v8：批改（答案不隨題庫下發）
...
/* v8 批改：整局結束時一次送出作答，回傳對錯、正解與解析。
   答案不再隨題庫下發，所以瀏覽器在作答期間沒有答案可查。
   刻意限制一次最多 GRADE_MAX 題，避免有人拿這個端點把整個題庫的答案撈走。 */
const GRADE_MAX = 25;

function grade_(p) {
  const raw = String(p.q || '').trim();
  if (!raw) return json_({ ok: false, error: 'no answers' });
  const items = raw.split(',').map(s => s.trim()).filter(String).map(s => {
    const i = s.lastIndexOf('.');
    return i < 0 ? { id: s, c: '' } : { id: s.slice(0, i), c: s.slice(i + 1).toLowerCase() };
  });
  if (items.length > GRADE_MAX) return json_({ ok: false, error: 'too many (max ' + GRADE_MAX + ')' });

  const ss = SpreadsheetApp.getActive();
  const out = [];
  ['OSH', 'ENV'].forEach(g => {
    const ids = items.filter(x => x.id.indexOf(g + '-') === 0);
    if (!ids.length) return;
    const sh = ss.getSheetByName('Questions_' + g);
    if (!sh) return;
    const head = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0].map(x => String(x).trim());
    const cId = head.indexOf('id') + 1, cAns = head.indexOf('answer') + 1;
    const cEz = head.indexOf('explain_zh') + 1, cEe = head.indexOf('explain_en') + 1;
    if (!cId || !cAns) return;
    const last = sh.getLastRow();
    ids.forEach(x => {
      // id 是依序編號的（OSH-0001…），所以列號可直接推算；推算錯了才退回整欄搜尋
      let row = parseInt(String(x.id).replace(/^[A-Z]+-/, ''), 10) + 1;
      if (!(row >= 2 && row <= last) || String(sh.getRange(row, cId).getValue()).trim() !== x.id) {
        const col = sh.getRange(2, cId, last - 1, 1).getValues();
        row = 0;
        for (let i = 0; i < col.length; i++) if (String(col[i][0]).trim() === x.id) { row = i + 2; break; }
        if (!row) { out.push({ id: x.id, error: 'not found' }); return; }
      }
      const ans = String(sh.getRange(row, cAns).getValue()).trim().toLowerCase();
      out.push({
        id: x.id, answer: ans, ok: (x.c && x.c === ans),
        explain_zh: cEz ? String(sh.getRange(row, cEz).getValue()).trim() : '',
        explain_en: cEe ? String(sh.getRange(row, cEe).getValue()).trim() : ''
      });
    });
  });
  return json_({ ok: true, version: GAS_VERSION, graded: out.length, results: out });
}
...
function buildBank_(status, group, fields) {
  ...
  rows.forEach(r => {
    ...
    const noExplain = (fields === 'core' || fields === 'play');
    need.forEach(k => { if (noExplain && (k === 'explain_zh' || k === 'explain_en')) return; if (fields === 'play' && k === 'answer') return; q[k] = r[idx[k]]; });
    q.difficulty = Number(q.difficulty) || 2;
    if (fields !== 'play') q.answer = ans;    // v8：play 模式不下發答案，改由 act=grade 於整局結束時批改
    q.status = st;
